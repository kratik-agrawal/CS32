...
City::City(int nRows, int nCols)
 : m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nFlatulans(0),m_h(nRows, nCols)
{
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    for(int i = m_player->row()-1; i <= m_player->row()+1; i++){
        for(int j = m_player->col()-1; j <= m_player->col()+1;j++){
            if(i == m_player->row() && j == m_player->col()){
                continue;
            }
            cnt += nFlatulansAt(i, j);
        }
    }
    int flatBefore = flatulanCount();
    ...
    if(flatulanCount() > (flatBefore-cnt)){
        if(cnt != 0)
            m_h.record(m_player->row(), m_player->col());
    }
}
